...
void RodExecutionPlan::RodExecutionUnit::Execute() {
  ...
  const tick_calc::SymbolRecordset<NbboPrice>& symbol_recordset = quote_mgr.LoadSymbolRecordset(date, symbol);
  ...
  for (auto rec : input_records) {
    ...
    try {
      ...
      if (rec.executions.empty()) {// no executions
        slices.emplace_back(rec.start_time, rec.end_time, rec.ord_qty);
      } else { // split order duration into execution count + 1 slices (start and end times, and leaves qty)
...
void RodExecutionPlan::Input(InputRecord& input_record) {
  ...
  const Double limit_price(input_record.values[argument_mapping[6]]);
  const RestType mpa = DecodeRestType(input_record.values[argument_mapping[7]]);
  InputRecordRange& input_range = input_record_ranges[make_pair(symbol, date)];
  input_range.emplace_back(input_record.id, start_time, end_time, side, ord_qty, limit_price, mpa);
  ...
  if (input_record.values.size() > 8 ) {
    if (input_record.values.size() % 2 == 1) {
      throw invalid_argument("Incomplete executions data");
    }
    auto & executions = input_range.rbegin()->executions;
    //ostringstream ss;
    for (size_t j = 8; j < input_record.values.size(); j += 2) {
